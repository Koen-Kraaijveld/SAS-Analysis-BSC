...
db = understand.open(
    ...
    r"3\SAS-Analysis-BSC\exemplars\Dragonfly\Dragonfly-master\DragonFly-Project\src\dragonfly.und")
...
def get_ent_array_from_names(ent_pool, ent_name_arr):
    ...
    for target_ent in ent_name_arr:
        for ent in ent_pool:
            if target_ent == ent.simplename() and ent.simplename() not in target_ents and "DroneBusinessObject" in ent.parent().name():
                target_ents.append(ent)

    return target_ents


adaptive_strategies = get_ent_array_from_names(func_ents, [
    "flying",
    "landing",
    "returnToHome",
    "safeLanding",
